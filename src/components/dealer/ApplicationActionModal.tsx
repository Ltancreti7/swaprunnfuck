...
import { DriverApplication, Driver } from '../../../shared/schema';
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {action === 'approve' ? (
            <div className="bg-green-50 border border-green-200 rounded-lg p-4 space-y-3">
              <p className="text-sm text-green-800 font-semibold">Driver Details</p>
              <div className="grid grid-cols-2 gap-2 text-sm text-green-700">
                ...
                <div>
                  <p className="font-medium text-green-800">Qualifications</p>
                  <p>{application.driver.canDriveManual ? 'Can drive manual' : 'Automatic only'}</p>
                  <p>{application.driver.radius} mile service radius</p>
                </div>
              </div>
              {application.driver.licenseNumber && (
              ...
              {application.message && (
                <div className="text-sm text-green-700 border-t border-green-200 pt-2">
                  <p className="font-medium text-green-800 mb-1">Application Message</p>
                  <p className="italic">"{application.message}"</p>
                </div>
              )}
              <div className="text-xs text-green-600 border-t border-green-200 pt-2">
                Applied {new Date(application.appliedAt).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}
              </div>
            </div>
          ) : (
            <div>
